...
```bash
git clone https://github.com/<your-username>/multimodal-ssl-pipeline.git
...
## Git workflow — pushing your submission

```bash
# 1. Initialise git (if not done yet)
git init
git remote add origin https://github.com/<your-username>/multimodal-ssl-pipeline.git

# 2. Stage only the code files (data/ and venv/ are excluded by .gitignore)
git add setup_data.sh setup.bat download_datasets.py
git add preprocess.py validate_outputs.py
git add configs/ tests/ reports/
git add requirements.txt README.md .gitignore

# 3. Stage the 100-sample submission packs (these ARE committed)
git add submission_sample/

# 4. Commit
git commit -m "Multimodal SSL preprocessing pipeline — MED05664 submission"

# 5. Push
git push -u origin main
```

> **What is NOT committed** (excluded by `.gitignore`):
> `data/raw/`, `data/interim/`, `data/processed/`, `venv/`, `logs/`
> These are regenerated by running the pipeline.

> **What IS committed:**
> All code, config, docs, tests, and the 100-sample submission packs in `submission_sample/`.
